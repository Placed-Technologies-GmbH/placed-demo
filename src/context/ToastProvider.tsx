...
export function ToastProvider({ children }: ToastProviderProps) {
  ...
  return (
    ...
    >
      ...
      {/* Toast Container */}
      <div className="fixed bottom-4 right-4 z-toast space-y-2 max-w-sm w-[350px]">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-medium text-text-sm">{toast.title}</h4>
                {toast.message && (
                  <p className="mt-1 text-text-xs opacity-90">{toast.message}</p>
                )}
              </div>
              <button
